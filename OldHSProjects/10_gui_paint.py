...
def draw(event):
  if shape.value == 'line':
    painting.line(painting.last_event.x, painting.last_event.y, event.x, event.y, color = color.value, width = width.value)
  else:
    if painting.last_shape is not None:
      painting.delete(painting.last_shape)

    if shape.value == 'rectangle':
      painting.last_shape = painting.rectangle(painting.first_event.x, painting.first_event.y, event.x, event.y, color = color.value)

    if shape.value == 'oval':
      painting.last_shape = painting.oval(painting.first_event.x, painting.first_event.y, event.x, event.y, color = color.value)
  painting.last_event = event
...
Text(tools, text = 'Color', align = 'left')
